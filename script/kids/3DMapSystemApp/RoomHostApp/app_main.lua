--[[
Title: RoomHostApp app for Paraworld
Author(s): LiXizhi
Date: 2008/1/5
Desc: 
...
function Map3DSystem.App.RoomHostApp.OnConnection(app, connectMode)
	if(connectMode == Map3DSystem.App.ConnectMode.UI_Setup) then
		...
		-- e.g. Create a RoomHostApp command link in the main menu 
		local commandName = "Profile.RoomHostApp";
		local command = Map3DSystem.App.Commands.GetCommand(commandName);
		if(command == nil) then
			--command = Map3DSystem.App.Commands.AddNamedCommand(
				--{name = commandName,app_key = app.app_key, ButtonText = "活动房间", icon = app.icon, });
		...
		-- e.g.
		Map3DSystem.App.RoomHostApp.app = app; -- keep a reference 
		app.HideHomeButton = true;
		app.HomeButtonText = "活动房间";
		app.about =  "Host room service application"
...
function Map3DSystem.App.RoomHostApp.OnDisconnection(app, disconnectMode)
	if(disconnectMode == Map3DSystem.App.DisconnectMode.UserClosed or disconnectMode == Map3DSystem.App.DisconnectMode.WorldClosed)then
		...
		local command = Map3DSystem.App.Commands.GetCommand("Profile.RoomHostApp");
		if(command == nil) then
			command:Delete();
		end
	end
...
function Map3DSystem.App.RoomHostApp.OnQueryStatus(app, commandName, statusWanted)
	if(statusWanted == Map3DSystem.App.CommandStatusWanted) then
		...
		if(commandName == "Profile.RoomHostApp") then
	...
	if(commandName == "Profile.RoomHostApp") then
		...
		if(_frame) then
			...
			local param = {
				...
				ShowUICallback = Map3DSystem.App.RoomHostApp.Show,
...
function Map3DSystem.App.RoomHostApp.MSGProc(window, msg)
	...
	if(msg.type == Map3DSystem.App.MSGTYPE.APP_CONNECTION) then	
		...
		Map3DSystem.App.RoomHostApp.OnConnection(msg.app, msg.connectMode);
		
	elseif(msg.type == Map3DSystem.App.MSGTYPE.APP_DISCONNECTION) then	
		-- Receives notification that the Add-in is being unloaded.
		Map3DSystem.App.RoomHostApp.OnDisconnection(msg.app, msg.disconnectMode);

	elseif(msg.type == Map3DSystem.App.MSGTYPE.APP_QUERY_STATUS) then
		-- This is called when the command's availability is updated. 
		-- NOTE: this function returns a result. 
		msg.status = Map3DSystem.App.RoomHostApp.OnQueryStatus(msg.app, msg.commandName, msg.statusWanted);
		
	elseif(msg.type == Map3DSystem.App.MSGTYPE.APP_EXEC) then
		-- This is called when the command is invoked.
		Map3DSystem.App.RoomHostApp.OnExec(msg.app, msg.commandName, msg.params);
				
	elseif(msg.type == Map3DSystem.App.MSGTYPE.APP_RENDER_BOX) then	
		-- Change and render the 3D world with mcml data that is usually retrieved from the current user's profile page for this application. 
		Map3DSystem.App.RoomHostApp.OnRenderBox(msg.mcml);
	...
	elseif(msg.type == CommonCtrl.os.MSGTYPE.WM_CLOSE) then
		Map3DSystem.UI.Windows.ShowWindow(false, Map3DSystem.App.RoomHostApp.app._app.name, msg.wndName);
...
function Map3DSystem.App.RoomHostApp.Show(bShow, _parent, parentWindow)
	...
	_this=ParaUI.GetUIObject("RoomHostAll_cont");
	if(_this:IsValid() == false) then
		if(bShow == false) then return	end
		bShow = true;
		
		if(_parent==nil) then
			_this=ParaUI.CreateUIObject("container","RoomHostAll_cont","_lt",100,50, 606, 390);
			_this:AttachToRoot();
		else
			_this = ParaUI.CreateUIObject("container", "RoomHostAll_cont", "_fi",0,0,0,0);
			_this.background = ""
