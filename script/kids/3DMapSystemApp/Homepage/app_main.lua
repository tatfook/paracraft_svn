--[[
Title: Homepage app for Paraworld
Author(s): LiXizhi
Date: 2008/1/31
Desc: 
...
function Map3DSystem.App.Homepage.OnConnection(app, connectMode)
	if(connectMode == Map3DSystem.App.ConnectMode.UI_Setup) then
		...
		Map3DSystem.App.Homepage.app = app; 
		
		-- e.g. Create a Homepage command 
		...
		if(command == nil) then
			command = Map3DSystem.App.Commands.AddNamedCommand(
				{name = commandName,app_key = app.app_key, ButtonText = "From Homepage UI Setup", icon = app.icon, });
		end
	end
end
...
function Map3DSystem.App.Homepage.OnDisconnection(app, disconnectMode)
	if(disconnectMode == Map3DSystem.App.DisconnectMode.UserClosed or disconnectMode == Map3DSystem.App.DisconnectMode.WorldClosed)then
		...
		local command = Map3DSystem.App.Commands.GetCommand("Profile.Homepage");
		if(command == nil) then
			command:Delete();
		end
	end
...
function Map3DSystem.App.Homepage.OnQueryStatus(app, commandName, statusWanted)
	if(statusWanted == Map3DSystem.App.CommandStatusWanted) then
		...
		if(commandName == "Profile.Homepage") then
...
function Map3DSystem.App.Homepage.OnExec(app, commandName, params)
	if(commandName == "Profile.Homepage") then
		...
		-- e.g.
		_guihelper.MessageBox("Homepage application executed. ");
...
function Map3DSystem.App.Homepage.MSGProc(window, msg)
	...
	if(msg.type == Map3DSystem.App.MSGTYPE.APP_CONNECTION) then	
		...
		Map3DSystem.App.Homepage.OnConnection(msg.app, msg.connectMode);
		
	elseif(msg.type == Map3DSystem.App.MSGTYPE.APP_DISCONNECTION) then	
		-- Receives notification that the Add-in is being unloaded.
		Map3DSystem.App.Homepage.OnDisconnection(msg.app, msg.disconnectMode);

	elseif(msg.type == Map3DSystem.App.MSGTYPE.APP_QUERY_STATUS) then
		-- This is called when the command's availability is updated. 
		-- NOTE: this function returns a result. 
		msg.status = Map3DSystem.App.Homepage.OnQueryStatus(msg.app, msg.commandName, msg.statusWanted);
		
	elseif(msg.type == Map3DSystem.App.MSGTYPE.APP_EXEC) then
		-- This is called when the command is invoked.
		Map3DSystem.App.Homepage.OnExec(msg.app, msg.commandName, msg.params);
				
	elseif(msg.type == Map3DSystem.App.MSGTYPE.APP_RENDER_BOX) then	
		-- Change and render the 3D world with mcml data that is usually retrieved from the current user's profile page for this application. 
		Map3DSystem.App.Homepage.OnRenderBox(msg.mcml);
