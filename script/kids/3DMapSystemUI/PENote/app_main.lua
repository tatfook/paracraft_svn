...
INSERT INTO apps VALUES (NULL, 'PENote_GUID', 'PENote', '1.0.0', 'http://www.paraengine.com/apps/PENote_v1.zip', 'YourCompany', 'enUS', 'script/kids/3DMapSystemUI/PENote/IP.xml', '', 'script/kids/3DMapSystemUI/PENote/app_main.lua', 'Map3DSystem.App.PENote.MSGProc', 1);
use the lib:
------------------------------------------------------------
NPL.load("(gl)script/kids/3DMapSystemUI/PENote/app_main.lua");
------------------------------------------------------------
]]
...
-- create class
local PENote = {};
commonlib.setfield("Map3DSystem.App.PENote", PENote);
-------------------------------------------
-- event handlers
-------------------------------------------
...
function Map3DSystem.App.PENote.OnConnection(app, connectMode)
	if(connectMode == Map3DSystem.App.ConnectMode.UI_Setup) then
		...
		-- e.g. Create a PENote command link in the main menu 
		local commandName = "Profile.PENote";
		local command = Map3DSystem.App.Commands.GetCommand(commandName);
		if(command == nil) then
			command = Map3DSystem.App.Commands.AddNamedCommand(
				{name = commandName,app_key = app.app_key, ButtonText = "消息", icon = "Texture/3DMapSystem/AppIcons/homepage_64.dds", });
		end
		...
		Map3DSystem.App.PENote.app = app; -- keep a reference
		if(ParaEngine.GetLocale() == "zhCN") then
			app.HomeButtonText = "PENote in Chinese";
		else
			app.HomeButtonText = "PENote in English";
		end
	end
end

-- Receives notification that the Add-in is being unloaded.
function Map3DSystem.App.PENote.OnDisconnection(app, disconnectMode)
	if(disconnectMode == Map3DSystem.App.DisconnectMode.UserClosed or disconnectMode == Map3DSystem.App.DisconnectMode.WorldClosed)then
		...
		local command = Map3DSystem.App.Commands.GetCommand("Profile.PENote");
		if(command == nil) then
			command:Delete();
		end
	end
...
function Map3DSystem.App.PENote.OnQueryStatus(app, commandName, statusWanted)
	if(statusWanted == Map3DSystem.App.CommandStatusWanted) then
		...
		if(commandName == "Profile.PENote") then
...
-- @param commandName: The name of the command to determine state for. Usually in the string format "Category.SubCate.Name".
function Map3DSystem.App.PENote.OnExec(app, commandName, params)
	
end

-- Change and render the 3D world with mcml data that is usually retrieved from the current user's profile page for this application. 
function Map3DSystem.App.PENote.OnRenderBox(mcmlData)
end


-- called when the user wants to nagivate to the 3D world location relavent to this application
function Map3DSystem.App.PENote.Navigate()
end

-- called when user clicks to check out the homepage of this application. Homepage usually includes:
-- developer info, support, developer worlds information, app global news, app updates, all community user rating, active users, trade, currency transfer, etc. 
function Map3DSystem.App.PENote.GotoHomepage()
end

-- called when user clicks the quick action for this application. 
function Map3DSystem.App.PENote.DoQuickAction()
end
...
function Map3DSystem.App.PENote.MSGProc(window, msg)
	...
	elseif(msg.type == Map3DSystem.App.MSGTYPE.APP_NAVIGATION) then
		...
		Map3DSystem.App.PENote.Navigate();
	...
	elseif(msg.type == Map3DSystem.App.MSGTYPE.APP_HOMEPAGE) then
		...
		Map3DSystem.App.PENote.GotoHomepage();
	
	elseif(msg.type == Map3DSystem.App.MSGTYPE.APP_QUICK_ACTION) then
		-- called when user clicks the quick action for this application. 
		Map3DSystem.App.PENote.DoQuickAction();
