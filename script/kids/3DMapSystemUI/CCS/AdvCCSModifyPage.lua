...
Author(s): WangTian
Date: 2009/5/5
Desc: script/kids/3DMapSystemUI/CCS/AdvCCSModifyPage.html
use the lib:
------------------------------------------------------------
NPL.load("(gl)script/kids/3DMapSystemUI/CCS/AdvCCSModifyPage.lua");
-------------------------------------------------------
]]
...
-- create class
local AdvCCSModifyPage = {};
commonlib.setfield("Map3DSystem.App.CCS.AdvCCSModifyPage", AdvCCSModifyPage);

-- on init show the current avatar in pe:avatar
function AdvCCSModifyPage.OnInit()
	...
	--if(db == nil) then
		...
		--return;
	--end
	--AdvCCSModifyPage._db = db;
end

function AdvCCSModifyPage.UpdateDragonSkin_Purple()
	AdvCCSModifyPage.UpdateDragonSkin(1)
end

function AdvCCSModifyPage.UpdateDragonSkin_Red()
	AdvCCSModifyPage.UpdateDragonSkin(2)
end

function AdvCCSModifyPage.UpdateDragonSkin_Green()
	AdvCCSModifyPage.UpdateDragonSkin(3)
end

function AdvCCSModifyPage.UpdateDragonSkin_Orange()
	AdvCCSModifyPage.UpdateDragonSkin(4)
end

function AdvCCSModifyPage.UpdateDragonSkin_DarkPurple()
	AdvCCSModifyPage.UpdateDragonSkin(5)
end

function AdvCCSModifyPage.UpdateDragonSkin_DarkRed()
	AdvCCSModifyPage.UpdateDragonSkin(6)
end

function AdvCCSModifyPage.UpdateDragonSkin(i)
...
function AdvCCSModifyPage.OnChangeAsset()
...
function AdvCCSModifyPage.ClickDBUpdate()
...
function AdvCCSModifyPage.ClickLeftHandUpdate(name, mcmlNode)
	if(mcmlNode) then
        local gsid = mcmlNode:GetNumber("gsid");
        if(gsid) then
			AdvCCSModifyPage.HandUpdate(gsid, 0);
		end
	end
end

function AdvCCSModifyPage.ClickRightHandUpdate(name, mcmlNode)
	if(mcmlNode) then
        local gsid = mcmlNode:GetNumber("gsid");
        if(gsid) then
			AdvCCSModifyPage.HandUpdate(gsid, 1);
		end
	end
end

function AdvCCSModifyPage.ClickHatUpdate(name, mcmlNode)
	if(mcmlNode) then
        local gsid = mcmlNode:GetNumber("gsid");
        if(gsid) then
...
function AdvCCSModifyPage.ClickBackUpdate(name, mcmlNode)
	if(mcmlNode) then
        local gsid = mcmlNode:GetNumber("gsid");
        if(gsid) then
			local playerChar = ParaScene.GetPlayer():ToCharacter();
...
function AdvCCSModifyPage.HandUpdate(gsid, hand)
	local playerChar = ParaScene.GetPlayer():ToCharacter();
...
function AdvCCSModifyPage.ClickAvaterUpdate(name, mcmlNode)
    if(mcmlNode) then
        local gsid = mcmlNode:GetNumber("gsid");
...
function AdvCCSModifyPage.DS_Func_Hairs(index)
	if(index ~= nil) then
...
function AdvCCSModifyPage.TestHair(style, color)
...
function AdvCCSModifyPage.DS_Func_BaseSkins(index)
	if(index ~= nil) then
		if(index == 1) then
...
function AdvCCSModifyPage.TestSkin(color)
	local player = ParaScene.GetPlayer();
	local playerChar = player:ToCharacter();
...
function AdvCCSModifyPage.TestEyeAddon(index)
...
function AdvCCSModifyPage.TestWings(index)
	index = tonumber(index);
	local player = ParaScene.GetPlayer();
	local playerChar = player:ToCharacter();
	if(index == 0) then
...
function AdvCCSModifyPage.GetDS_Func_CartoonFace(index_Func)
...
function AdvCCSModifyPage.TestCartoonFace(type, style)
...
function AdvCCSModifyPage.Custom_ComposedFace(params)
	...
	AdvCCSModifyPage.ClickCartoonFaceComponent("Eye");
end

function AdvCCSModifyPage.ClickCartoonFaceComponent(value)
...
function AdvCCSModifyPage.GetDS_Func_CharacterSlot(index_Func)
	return function (index)
		if(index ~= nil) then
			if(index_Func == 1) then
				-- head
...
function AdvCCSModifyPage.TestCharacterSlot(type, style)
...
--
--function AdvCCSModifyPage.Custom_ComposedSkin(params)
--end



--function AdvCCSModifyPage.TestNameConvention()
...
function AdvCCSModifyPage.TakeAvatarSnapshot()
...
function AdvCCSModifyPage.OnRefreshAvatar()
	local self = document:GetPageCtrl();
	if(not self) then 
		log("warning: page control not found\n")
		return 
...
function AdvCCSModifyPage.OnClickSave()
...
function AdvCCSModifyPage.Custom_OriginalCCSMain(params)
