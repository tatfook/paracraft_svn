--[[
Title: Main bar in InGame UI for 3D Map system
Author(s): WangTian
Date: 2007/9/17
Desc: Show the main bar in game UI
use the lib:
------------------------------------------------------------
NPL.load("(gl)script/kids/3DMapSystemUI/InGame/MainBar.lua");
...
--		and file: script/kids/3DMapSystemData/MainBarData.lua
------------------------------------------------------------


-- themeable mainbar style UI information
Map3DSystem.UI.MainBar.iconDefaultSize = 48;
Map3DSystem.UI.MainBar.iconHeightOffset = 12;
Map3DSystem.UI.MainBar.height = 64;
Map3DSystem.UI.MainBar.background = "Texture/Themes/Reflection/MainBar/MainBarBG.png";
Map3DSystem.UI.MainBar.bgWidthAdditional = 32;
Map3DSystem.UI.MainBar.separatorWidth = 8;
Map3DSystem.UI.MainBar.separatorBG = "Texture/Themes/Reflection/MainBar/Separator.png";

-- grid display style
Map3DSystem.UI.MainBar.gridSpacingLeft = 16;
Map3DSystem.UI.MainBar.gridSpacingRight = 16;
Map3DSystem.UI.MainBar.gridSpacingTop = 16;
Map3DSystem.UI.MainBar.gridSpacingBottom = 16;

Map3DSystem.UI.MainBar.gridCellSizeX = 48;
Map3DSystem.UI.MainBar.gridCellSizeY = 48;
Map3DSystem.UI.MainBar.gridCellGapX = 4;
Map3DSystem.UI.MainBar.gridCellGapY = 4;

-- stack display style
Map3DSystem.UI.MainBar.stackCellSizeX = 48;
Map3DSystem.UI.MainBar.stackCellSizeY = 48;
Map3DSystem.UI.MainBar.stackCellGapHeight = 4;
Map3DSystem.UI.MainBar.stackGapHeight = 8;

---- category display style
--Map3DSystem.UI.MainBar.categoryCellSizeX = 48;
--Map3DSystem.UI.MainBar.categoryCellSizeY = 48;
--Map3DSystem.UI.MainBar.categoryCellGapHeight = 4;
--Map3DSystem.UI.MainBar.categoryGapHeight = 8;




-- init main bar:
function Map3DSystem.UI.MainBar.InitMainBar()
	
	commonlib.echo("Map3DSystem.UI.MainBar.InitMainBar()");
	...
	if(not Map3DSystem.UI.MainBar.bIsInit) then
		...
		file = UIAnimManager.LoadUIAnimationFile(fileName);
		fileName = "script/UIAnimation/CommonProgress.lua.table";
		file = UIAnimManager.LoadUIAnimationFile(fileName);
	
	
		Map3DSystem.UI.MainBar.InitMessageSystem();
		--Map3DSystem.UI.MainBar.TestGroups();
		--Map3DSystem.UI.MainBar.Show(true);
		...
		-- by LXZ 2008.2.8 it is read from asset app. 
	...
	Map3DSystem.UI.MainBar.Show(false);
	------------------------------------------------
end
...
-- item area size in X and Y
Map3DSystem.UI.MainBar.defaultItemAreaWidth = 48;
Map3DSystem.UI.MainBar.defaultItemAreaHeight = 64;
-- separator width, separator height is the same as itemAreaHeight
Map3DSystem.UI.MainBar.separatorWidth = 16;
-- side width is the additional left and right width of bar space besides the groups and separators
Map3DSystem.UI.MainBar.sideWidth = 48;


-- show the main bar in dock form
-- this is one time init function to world scene
function Map3DSystem.UI.MainBar.Show()
	NPL.load("(gl)script/ide/Dock.lua");
	local ctl = CommonCtrl.GetControl("PW_MainBar");
	if(ctl == nil)then
		ctl = CommonCtrl.Dock:new{
			name = "PW_MainBar",
			parent = nil,
			height = 64,
			--zorder = 1,
			background = "Texture/3DMapSystem/MainBar.png: 48 8 48 8",
			separatorBackground = "Texture/3DMapSystem/Separator2.png",
			bUseAnimation = false,
		};
		
		-- init the basic groups and items in main bar
		Map3DSystem.UI.MainBar.InitGroups();
	end
	
	-- NOTE: little trick here to avoid direct animation setting first and set animation back
	ctl.bUseAnimation = false;
	ctl:Show();
	ctl.bUseAnimation = true;
end

-- get main bar item ui container
-- @param: group: group index or group name
-- @param: item: item index or item name
function Map3DSystem.UI.MainBar.GetItemUIContainer(group, item)
	local ctl = CommonCtrl.GetControl("PW_MainBar");
	if(ctl ~= nil) then
		return ctl:GetItemUIContainer(group, item);
	end
end

-- init the basic groups and items in main bar
-- Currently including: "apps", "stack" and "feeds"
function Map3DSystem.UI.MainBar.InitGroups()
	local ctl = CommonCtrl.GetControl("PW_MainBar");
	if(ctl ~= nil) then
		ctl:InsertGroup("apps", 1);
		ctl:InsertGroup("stacks", 2);
		ctl:InsertGroup("feeds", 3);
	end
end

-- Insert a main bar command at given position. The group must be created beforehand, 
--		otherwise it will be inserted to the "stacks" folder.
...
-- @param groupName: group name currently including: "apps", "stack" and "feeds"
-- @param itemName: item name
-- @param itemIndex: position of the item in the group, starts from 1
...
function Map3DSystem.UI.MainBar.InsertBarItem(command, groupName, itemName, itemIndex, forceUpdate, forceDisableAnimation)
	
	local ctl = CommonCtrl.GetControl("PW_MainBar");
	if(ctl ~= nil) then
		local groupIndex = ctl:GetGroupIndexByName(groupName);
		if(groupIndex == nil) then
			-- group not found
			groupIndex = ctl:GetGroupIndexByName("stacks");
		end
		ctl:InsertItemToGroup(itemName, itemIndex, groupIndex, command, nil, forceUpdate, forceDisableAnimation);
	end
end

-- Remove a main bar command. The command and group must be created beforehand, 
function Map3DSystem.UI.MainBar.RemoveBarItem(itemName, groupName)
	...
	if(ctl ~= nil) then
		...
		ctl:RemoveItemFromGroupByName(itemName, groupIndex);
		ctl:Update();
	end
end

--function Map3DSystem.UI.MainBar.Show(bShow, parentWindow)
	--local _this;
	---- TODO: save parentWindow
	--_this = ParaUI.GetUIObject("MainBar_cont");
	--if(_this:IsValid() == false) then
		...
		--local barWidth = 0;
		--local separatorWidth = Map3DSystem.UI.MainBar.separatorWidth;
		--
		---- left side width
		--barWidth = barWidth + Map3DSystem.UI.MainBar.sideWidth;
		--
		---- themeable main bar
		--local _mainbar = ParaUI.CreateUIObject("container", "MainBar_cont", "_ctb", 0, 0, barWidth, 64);
		----_mainbar.background = ""; --  TODO: main bar background
		--_mainbar:AttachToRoot();
		--
		--local k, v;
		--for k, v in ipairs(Map3DSystem.UI.MainBar.groups) do
			--if(type(k) == "number") then
				--local groupWidth = 0;
				--local _group = ParaUI.CreateUIObject("container", "group_"..k, "_lt", barWidth, 0, groupWidth, 64);
				----_group.background = ""; --  TODO: group background
				--_mainbar:AddChild(_group);
				--local kk, vv;
				--for kk, vv in ipairs(v) do
					--if(type(kk) == "number") then
						--local _item = ParaUI.CreateUIObject("container", "item_"..kk, "_lt", groupWidth, 0, vv.itemAreaWidth, 64);
						----_group.background = ""; --  TODO: group background
				...
				--
				--if(Map3DSystem.UI.MainBar.groups[k + 1] ~= nil) then
					--if(Map3DSystem.UI.MainBar.groups[k + 1][1] ~= nil) then
						---- exist group and unempty group
						--local _separator = ParaUI.CreateUIObject("container", "separator_"..k, "_lt", barWidth, 0, separatorWidth, 64);
						----_separator.background = ""; --  TODO: separator background
						--_mainbar:AddChild(_separator);
		...
		--barWidth = barWidth + Map3DSystem.UI.MainBar.sideWidth;
		--
		---- reset the main bar width
		--_mainbar.width = barWidth;
	--else
		--if(bShow == nil) then
...
function Map3DSystem.UI.MainBar.InitUI()
	...
	-- themeable mainbar style UI information
	local _iconDefaultSize = Map3DSystem.UI.MainBar.iconDefaultSize;
	local _iconHeightOffset = Map3DSystem.UI.MainBar.iconHeightOffset;
	local _height = Map3DSystem.UI.MainBar.height;
	local _background = Map3DSystem.UI.MainBar.background;
	local _bgWidthAdditional = Map3DSystem.UI.MainBar.bgWidthAdditional;
	local _separatorWidth = Map3DSystem.UI.MainBar.separatorWidth;
	local _separatorBG = Map3DSystem.UI.MainBar.separatorBG;
	...
	-- this make it on top of normal windows, but still under all top level windows. 	
	_bar:AttachToRoot();
...
function Map3DSystem.UI.MainBar.OnClickIcon(index)
	...
	local _icon;
	if(index == 1) then
		_icon = Map3DSystem.UI.MainBar.GetItemUIContainer("apps", "Creator.Main");
	end
...
function Map3DSystem.UI.MainBar.SwitchToStatus(status)
	...
	CommonCtrl.os.hook.Invoke(CommonCtrl.os.hook.HookType.WH_CALLWNDPROC, 0, "MainBar", msg);
end
