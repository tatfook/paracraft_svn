...
void vs_main(
    ...
    out float3 halfVector:TEXCOORD4)
{
	float3 worldPos = mul(pos,mWorld);
	pos = mul(pos,mWorldViewProj);
	float3 viewDir = normalize(g_EyePositionW - worldPos);
...
void ps_main(in float2 texCoord: TEXCOORD0,
	...
	in float3 halfVector:TEXCOORD4,
	out half4 color :COLOR)
{
	...
	tempTexCoord = (texCoord  + finalWaveBump.xy * 0.3 + texCoordOffset.yz) * texCoordOffset.x;
