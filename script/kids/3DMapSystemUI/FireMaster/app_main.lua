...
use the lib:
------------------------------------------------------------
NPL.load("(gl)script/kids/3DMapSystemUI/FireMaster/app_main.lua");
------------------------------------------------------------
]]
...
-- create class
local FireMaster = {};
commonlib.setfield("Map3DSystem.App.FireMaster", FireMaster);
-------------------------------------------
-- event handlers
-------------------------------------------
...
function Map3DSystem.App.FireMaster.OnConnection(app, connectMode)
	if(connectMode == Map3DSystem.App.ConnectMode.UI_Setup) then
		...
		-- e.g. Create a FireMaster command link in the main menu 
		local commandName = "Profile.FireMaster";
		...
		Map3DSystem.App.FireMaster.app = app; -- keep a reference
		if(ParaEngine.GetLocale() == "zhCN") then
			app.HomeButtonText = "FireMaster in Chinese";
		else
			app.HomeButtonText = "FireMaster in English";
		end
	end
end

-- Receives notification that the Add-in is being unloaded.
function Map3DSystem.App.FireMaster.OnDisconnection(app, disconnectMode)
	if(disconnectMode == Map3DSystem.App.DisconnectMode.UserClosed or disconnectMode == Map3DSystem.App.DisconnectMode.WorldClosed)then
		...
		local command = Map3DSystem.App.Commands.GetCommand("Profile.FireMaster");
		if(command == nil) then
			command:Delete();
		end
	end
...
function Map3DSystem.App.FireMaster.OnQueryStatus(app, commandName, statusWanted)
	if(statusWanted == Map3DSystem.App.CommandStatusWanted) then
		...
		if(commandName == "Profile.FireMaster") then
...
-- @param commandName: The name of the command to determine state for. Usually in the string format "Category.SubCate.Name".
function Map3DSystem.App.FireMaster.OnExec(app, commandName, params)
	
end

-- Change and render the 3D world with mcml data that is usually retrieved from the current user's profile page for this application. 
function Map3DSystem.App.FireMaster.OnRenderBox(mcmlData)
end


-- called when the user wants to nagivate to the 3D world location relavent to this application
function Map3DSystem.App.FireMaster.Navigate()
end

-- called when user clicks to check out the homepage of this application. Homepage usually includes:
-- developer info, support, developer worlds information, app global news, app updates, all community user rating, active users, trade, currency transfer, etc. 
function Map3DSystem.App.FireMaster.GotoHomepage()
end

-- called when user clicks the quick action for this application. 
function Map3DSystem.App.FireMaster.DoQuickAction()
end
...
-----------------------------------------------------
function Map3DSystem.App.FireMaster.MSGProc(window, msg)
	----------------------------------------------------
	-- application plug-in messages here
	----------------------------------------------------
	if(msg.type == Map3DSystem.App.MSGTYPE.APP_CONNECTION) then	
		-- Receives notification that the Add-in is being loaded.
		Map3DSystem.App.FireMaster.OnConnection(msg.app, msg.connectMode);
		
	elseif(msg.type == Map3DSystem.App.MSGTYPE.APP_DISCONNECTION) then	
		-- Receives notification that the Add-in is being unloaded.
		Map3DSystem.App.FireMaster.OnDisconnection(msg.app, msg.disconnectMode);

	elseif(msg.type == Map3DSystem.App.MSGTYPE.APP_QUERY_STATUS) then
		-- This is called when the command's availability is updated. 
		-- NOTE: this function returns a result. 
		msg.status = Map3DSystem.App.FireMaster.OnQueryStatus(msg.app, msg.commandName, msg.statusWanted);
		
	elseif(msg.type == Map3DSystem.App.MSGTYPE.APP_EXEC) then
		-- This is called when the command is invoked.
		Map3DSystem.App.FireMaster.OnExec(msg.app, msg.commandName, msg.params);
				
	elseif(msg.type == Map3DSystem.App.MSGTYPE.APP_RENDER_BOX) then	
		-- Change and render the 3D world with mcml data that is usually retrieved from the current user's profile page for this application. 
		Map3DSystem.App.FireMaster.OnRenderBox(msg.mcml);
	...
	elseif(msg.type == Map3DSystem.App.MSGTYPE.APP_NAVIGATION) then
		...
		Map3DSystem.App.FireMaster.Navigate();
	...
	elseif(msg.type == Map3DSystem.App.MSGTYPE.APP_HOMEPAGE) then
		...
		Map3DSystem.App.FireMaster.GotoHomepage();
	
	elseif(msg.type == Map3DSystem.App.MSGTYPE.APP_QUICK_ACTION) then
		-- called when user clicks the quick action for this application. 
		Map3DSystem.App.FireMaster.DoQuickAction();
