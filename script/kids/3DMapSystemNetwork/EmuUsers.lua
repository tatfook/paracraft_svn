...
function user:LoginJGSL()
	if(not (self.server and self.playeragent and self.playeragent.x and self.playeragent.AssetFile)) then
		commonlib.log("JCEmu: JGSL is ignored, since there is no agent or server data for %s \n", self.jid);
		return;
	end
	if(not self.client) then
		self.client = Map3DSystem.JGSL.client:new ({IsEmulated=true, jid = self.jid})
	end
	...
	commonlib.log("JCEmu: %s connecting to JGSL gateway %s\n", self.jid, self.server)
