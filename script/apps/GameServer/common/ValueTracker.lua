...
local vtracker = Map3DSystem.GSL.ValueTracker:new()
...
local ValueTracker = commonlib.gettable("Map3DSystem.GSL.ValueTracker");

-- default size is 2, which keeps just one history item. 
-- @param o: nil or something like {size=10}
...
-- rollback to last history data, but the time remain the same. 
function ValueTracker:Rollback(last_time, last_value)
	self:Update(last_time or self:GetTime(-1), last_value or self:GetValue(-1));
end
