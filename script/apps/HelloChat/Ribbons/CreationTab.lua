...
Use Lib:
-------------------------------------------------------
NPL.load("(gl)script/apps/HelloChat/Desktop/CreationTab.lua");
-------------------------------------------------------
]]
NPL.load("(gl)script/ide/Encoding.lua");
local CreationTab = {};
commonlib.setfield("MyCompany.HelloChat.CreationTab", CreationTab)
...
function CreationTab.OnInit()
	...
	commonlib.setfield("MyCompany.HelloChat.RibbonControl.OnShowCreationTab", function (bShow)
		if(bShow) then
		...
		if(obj) then 
			...
			if(not CreationTab.rcEditObjTabs) then
				...
				CreationTab.rcEditObjTabs = CommonCtrl.RibbonControl:new({
					name = "HelloChat.Creation.SelectRibbonControl",
					alignment = "_lb",
					left = 350+3,
					top = -35-46-38-4,
					width = 500,
					...
					parent = nil,
					tabs = {
						["CharEditTab"] = {file="script/apps/HelloChat/Ribbons/CharEditTab.html", onshow=nil, },
						["ModelEditTab"] = {file="script/apps/HelloChat/Ribbons/ModelEditTab.html", onshow=nil, },
					},
				});
			end
			...
			if(obj:IsCharacter()) then
				...
				-- update name
				local _this = ParaUI.GetUIObject("hellochat_edit_char_objname");
				...
				-- update name
				local _this = ParaUI.GetUIObject("hellochat_edit_model_objname");
				if(_this:IsValid()) then
					local name, assetname = GetAssetDisplayName(obj)
					_this.text, _this.tooltip = name, "点击查看:"..tostring(assetname);
				end
...
function CreationTab.OnSelectionInfo()
	System.App.Commands.Call("File.MCMLWindowFrame", {
		url="script/apps/HelloChat/Assets/AssetInfoViewer.html",
		name = "HelloModelViewer",
		app_key = MyCompany.HelloChat.app.app_key, 
		text = "模型信息",
