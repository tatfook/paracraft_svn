...
Author(s): LiXizhi
Date: 2008/10/27
Desc:  script/apps/HelloChat/Desktop/LoginPage.html?cmdredirect=Profile.HomePage
Use Lib:
-------------------------------------------------------
NPL.load("(gl)script/apps/HelloChat/Desktop/LoginPage.lua");
MyCompany.HelloChat.LoginPage.Proc_Authentication(values, funcCallBack, bSkipAppRegistration)
-------------------------------------------------------
]]
local L = CommonCtrl.Locale("ParaWorld");

local LoginPage = {};
commonlib.setfield("MyCompany.HelloChat.LoginPage", LoginPage)
...
function LoginPage.Proc_Authentication(values, funcCallBack, bSkipAppRegistration)
	...
	paraworld.auth.AuthUser(msg, "login", function (msg)
		...
		elseif(msg.issuccess) then	
			...
			if(not MyCompany.HelloChat.app:ReadConfig("Registered", false)) then
				MyCompany.HelloChat.app:WriteConfig("Registered", true);
			end
			if(values.rememberpassword) then
				System.User.SaveCredential(System.User.Name, System.User.Password);
			end
			MyCompany.HelloChat.app:WriteConfig("rememberpassword", values.rememberpassword);
