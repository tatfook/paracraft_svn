...
function GlobalStorePage.DS_Func_Head(index)
	if(index ~= nil) then
		-- get the entries in the global store by category
		local itemEntries = ItemManager.GlobalStoreItemsByCategory["Head"];
		local gsID = itemEntries[index];
		return ItemManager.GlobalStoreItems[gsID];
	elseif(index == nil) then
		return #(ItemManager.GlobalStoreItemsByCategory["Head"]);
	end
end

-- purchase the item directly from global store
function GlobalStorePage.PurchaseItem(gsID)
	-- getitem in global store 
	local itemTemplate = ItemManager.ReadGlobalStoreItem(gsID);
	if(itemTemplate ~= nil) then
		-- put the item in the last position in the inventory category
		commonlib.echo(gsID);
	end
end