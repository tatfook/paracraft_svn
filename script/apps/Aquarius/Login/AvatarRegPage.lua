...
Author(s): WangTian
Date: 2008/3/18, revised 2008.3.21 LXZ re
use the lib:
------------------------------------------------------------
NPL.load("(gl)script/apps/Aquarius/Login/AvatarRegPage.lua");
-------------------------------------------------------
]]
...
-- create class
local AvatarRegPage = System.mcml.PageCtrl:new({url="script/apps/Aquarius/Login/AvatarRegPage.html"});

commonlib.setfield("MyCompany.Aquarius.Login.AvatarRegPage", AvatarRegPage)
...
function AvatarRegPage.UpdateAvatar(index)
	...
	local uid = System.App.profiles.ProfileManager.GetUserID() or "";
	...
	if(_slash ~= nil) then
		...
		if(string.lower(gender) ~= "female" and string.lower(gender) ~= "male") then
			...
			modelPath = string.format([[character/v3/%s/%s/%s%s.xml]], race, gender, race, gender);
		end
...
function AvatarRegPage.OnAccept(btnName, values, bindingContext)
	...
	if(race == nil or gender == nil) then
		...
		if(AvatarRegPage.CharParams) then
			...
			local profile = System.App.CCS.app:GetMCMLInMemory() or {};
			...
			if(not commonlib.partialcompare(profile.CharParams, AvatarRegPage.CharParams)) then
				...
				System.App.CCS.app:SetMCML(nil, profile, function (uid, appkey, bSucceed)
					if(bSucceed) then
						paraworld.ShowMessage("更新成功！准备进入公共世界")
						--self:Leave();
						MyCompany.Aquarius.LoginPage.Proc_PrepareEnterWorld();
					else
