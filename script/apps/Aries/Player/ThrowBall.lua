...
local validmodels = {
["model/07effect/v5/WaterBalloon/WaterBalloon1.x"] = true,
["model/07effect/v5/Firecracker/Firecracker1.x"] = true,
["model/07effect/v5/Jelly/Jelly1.x"] = true,
["model/06props/shared/pops/barrels.x"] = true,
--fishing net
["model/02furniture/v5/FishingTerrace/Fishnet/Fishnet.x"] = true,
["model/02furniture/v5/FishingTerrace/Fishnet/Fishnet01.x"] = true,
["model/02furniture/v5/FishingTerrace/Fishnet/Fishnet02.x"] = true,
["model/02furniture/v5/FishingTerrace/Fishnet/Fishnet03.x"] = true,
["model/02furniture/v5/FishingTerrace/Fishnet/Fishnet04.x"] = true,
-- pics
["Texture/Aries/Smiley/face10_32bits.png"] = true,
["Texture/Aries/Smiley/face03_32bits.png"] = true,
["Texture/Aries/Smiley/face15_32bits.png"] = true,
["Texture/Aries/Smiley/animated/face10_32bits_fps10_a005.png"] = true,
}
...
function ThrowBall.ValidateThrowItem(throwItem)
	if(not throwItem) then
		return true;
	end
	local ballStyle;
	if(throwItem.gsid) then
		if(ThrowablePage.ballList[throwItem.gsid]) then
			...
			ballStyle = gsItem.assetfile;
			throwItem.style = ballStyle;
		elseif(throwItem.gsid == 0) then
			-- LOG.std(nil, "System", "ThrowBall", "the %s throw a fish net.", tosting(nid or ""));
	...
	if(not throwItem.gsid or throwItem.gsid==0 and throwItem.style) then
		-- validate ballStyle
		if(not validmodels[throwItem.style]) then
			return 
		end
	end
	if(throwItem.hitstyle and throwItem.hitstyle~="") then
		if(not validmodels[throwItem.hitstyle]) then
			return 
		end
	end
	if(throwItem.showpic and throwItem.showpic~="") then
		if(not validmodels[throwItem.showpic]) then
			return 
		end
	end
	return true, ballStyle;
end
...
function ThrowBall.HandleMessage(msg)
	...
	local bValid, ballStyle = ThrowBall.ValidateThrowItem(throwItem);
	if(not bValid)then
		return 
	end
	...
	if(startPoint and ballStyle)then
