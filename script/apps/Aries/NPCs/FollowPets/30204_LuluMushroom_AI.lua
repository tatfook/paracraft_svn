...
Author: WangTian
Date: 2009/8/25
Desc: LuluMushroom AI

script/apps/Aries/NPCs/FollowPets/30204_LuluMushroom_AI.lua
...
-- create class
local libName = "LuluMushroom_AI";
local LuluMushroom_AI = commonlib.gettable("MyCompany.Aries.Quest.NPCAI.LuluMushroom_AI");

-- LuluMushroom_AI framemove
function LuluMushroom_AI.On_FrameMove()
	...
	if(mushroom:IsValid() == true and player:IsValid() == true) then
		...
		if(memory.hit_gsid == 9501 or memory.hit_gsid == 9506) then
		...
		if(memory.isFull == true) then
			...
			if(dist <= 5) then
				return;
				---- skip random walk
				--if(targetNPC_id == 30204 and targetNPC_instance ~= instance) then
