...
Author: WangTian
Date: 2009/8/25
Desc: ZodiacAnimals AI

script/apps/Aries/NPCs/FollowPets/30212_ZodiacAnimals_AI.lua
...
-- create class
local libName = "ZodiacAnimals_AI";
local ZodiacAnimals_AI = commonlib.gettable("MyCompany.Aries.Quest.NPCAI.ZodiacAnimals_AI");

-- ZodiacAnimals_AI framemove
function ZodiacAnimals_AI.On_FrameMove()
	-- 0.3s interval
	...
	if(chick:IsValid() == true and player:IsValid() == true) then
		...
		if(memory.dist and memory.dist > 5 and dist <= 5) then
			...
			if(targetNPC_id == 30212 and targetNPC_instance ~= instance) then
		...
		local memory_full = MyCompany.Aries.Quest.NPCAIMemory.GetMemory(30212);
