...
use the lib:
------------------------------------------------------------
NPL.load("(gl)script/apps/Aries/NPCs/Police/30108_KingKongSnail_AI.lua");
------------------------------------------------------------
]]
NPL.load("(gl)script/apps/Aries/Quest/NPCAIMemory.lua");

-- create class
local KingKongSnail_AI = commonlib.gettable("MyCompany.Aries.Quest.NPCAI.KingKongSnail_AI");
...
function KingKongSnail_AI.On_FrameMove()
	-- 0.3s interval
	...
	if(thisNPC:IsValid()) then 
		...
		local memory = NPCAIMemory.GetMemory(30108);
