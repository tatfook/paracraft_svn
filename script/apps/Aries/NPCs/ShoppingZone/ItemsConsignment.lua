...
local MAX_PRICE = 9999999
local COMMISSION_FACTOR  = 0.2
local PAGE_SIZE_PURCHASE = 11
local PAGE_SIZE_SELL = 11;
ItemsConsignment.SELL_ID = 0;
ItemsConsignment.MAGICBEAN_ID = 984
...
ItemsConsignment.MiddlePriceTable = {[3] = 240,[5] = 400,[7] = 560,};

local function GetMiddlePrice(arg)
	return ItemsConsignment.MiddlePriceTable[tonumber(arg) or 3];
end
...
{value="显示",selected = false,},
{value="不显示",selected = true,},};
...
function ItemsConsignment:Init()
	...
	if(AvatarBag.Items and #AvatarBag.Items > 0)then
		...
		ItemsConsignment.page:SetValue("txtCommission",GetMiddlePrice(ItemsConsignment.goods.expire))
	end

	-- self.page:SetValue("incomingItem",self.goods.gsid or -999);
end

function ItemsConsignment.GetItemCopies(gsid)
...
function ItemsConsignment.Filter(arg)
	...
	if(arg == 0 )then
		...
		if(ItemsConsignment.goods.gsid ~= -999 or ItemsConsignment.goods.gsid)then
			...
			ItemsConsignment.goods.gsid = -999
		end
	else
		ItemsFilter:Clean()
...
function ItemsConsignment.GetName()
	...
	local item_name = "";
	if(self.goods.gsid and self.goods.gsid ~= -999)then
...
function ItemsConsignment.ShowPage(tabName)
	...
	local self = ItemsConsignment;
	do 
		--MyCompany.Aries.Desktop.Dock.FireCmd("AuctionHouse.ShowPage", "sell", true);
		local is_market;
		if(tabName == "sell" or tabName == "buy") then
			is_market = true;
		end
		MyCompany.Aries.Desktop.Dock.FireCmd("AuctionHouse.ShowPage", tabName, is_market);
		return
	end
	...
	local width,height = 730,490;

	local params = {
        url = "script/apps/Aries/NPCs/ShoppingZone/ItemsConsignment.html", 
...
function ItemsConsignment.OnClickItem(arg)
	ItemsConsignment.goods = {copies = 0,guid=0,gsid=-999,price = nil}
...
///     psize 每页的数据量
	...
	if( ItemsConsignment.filter == 0)then
		...
		local txtMinLevel = ItemsConsignment.page:FindControl("txtMinLevel")
		if(txtMinLevel and txtMinLevel.text ~= "")then

			if(not tonumber(txtMinLevel.text))then
			
			else
				minlel = tonumber(txtMinLevel.text);
			end
		
		end

		local txtMaxLevel = ItemsConsignment.page:FindControl("txtMaxLevel")
		if(txtMaxLevel and txtMaxLevel.text ~= "")then

			if(not tonumber(txtMaxLevel.text))then

			else
				maxlel = tonumber(txtMaxLevel.text);
			end
		...
		for i,v in ipairs(self.QtyTable)do
			if(v.selected)then
				if(v.value == "普通")then
					quality = 0
				elseif(v.value == "精良")then
					quality = 1
				elseif(v.value == "卓越")then
					quality = 2
				elseif(v.value == "极品")then
					quality = 3
				else
		...
		--echo("++++++++++SearchFromShop+++++++++++")
		--ItemsConsignment.SearchFromShop({minlel = minlel,maxlel=maxlel,itemclass= ItemsFilter.class,subclass=ItemsFilter.subclass,quality=quality,gsname=name,pindex=ItemsConsignment.pindex,psize=PAGE_SIZE_PURCHASE});
	...
	--elseif(ItemsConsignment.prev_call == "GetInShop" and ItemsConsignment.filter == 0)then
		--ItemsConsignment.GetInShop({itemclass= ItemsFilter.class,subclass=ItemsFilter.subclass,school=ItemsFilter.school_code,pindex=ItemsConsignment.pindex,psize=PAGE_SIZE_PURCHASE});
...
function ItemsConsignment.BeginSell()
	if(not ItemsConsignment.goods.gsid or ItemsConsignment.goods.gsid == -999)then MSG("请先放入要寄售的物品！");return end
	if( not ItemsConsignment.goods.price)then MSG("你还没有指定出售价格！"); return end
	if(ItemsConsignment.goods.price == 0 or not tonumber(ItemsConsignment.goods.price))then MSG("无效的卖价，价格是数字而且应该大于0！");return end
	if(ItemsConsignment.GetItemCopies(ItemsConsignment.SELL_ID) < (ItemsConsignment.goods.expire +1))then MSG("你的金豆不足以支付寄售费!"); return end


	_guihelper.MessageBox(string.format("你确定以%s魔豆寄售物品【%s】么？（注：寄售%s天， 手续费%s奇豆）",
	...
	GetMiddlePrice(ItemsConsignment.goods.expire)), function(result)
...
function ItemsConsignment.SellPrice(ctrl)
	...
	if(ctrl)then
		local v = tonumber(ctrl.text);
		if(not v or v > MAX_PRICE)then
			self.goods.price = 1;
			self.page:SetUIValue("txtSellPrice",self.goods.price)
			if(v > MAX_PRICE)then
				MSG("物品售价不能太大。");
			end
			return;
...
function ItemsConsignment:Clean()
	...
	ItemsFilter:Clean()
	ItemsConsignment.goods = {copies = 0,guid=0,gsid=-999}
...
function ItemsConsignment:Update(list,...)
		...
		for i,v in ipairs(list)do
			...
			if(arg1 == "面包")then
				if(v.gsid <= 17140 and v.gsid >= 17131)then
					...
					price=v.price,time=time,nname=seller,unformattedtime = v.expire,
					copies = v.cnt,unit = unit,qty = qty,
					is_selected=is_selected,typ=typ,class=cn_school or "",})
				end

			elseif(arg1 == "治疗药剂")then
				if(v.gsid == 17155 or v.gsid == 17156)then
				table_insert(self.DisplayItems,{id=v.id,gsid = v.gsid,name=item_name,
				level = level,nid=v.nid,
				price=v.price,time=time,nname=seller,unformattedtime = v.expire,
				copies = v.cnt,unit = unit,qty = qty,
			...
			elseif(arg1 == "货币")then
				if(v.gsid == 17168 or v.gsid == 17144)then
				table_insert(self.DisplayItems,{id=v.id,gsid = v.gsid,name=item_name,
				level = level,nid=v.nid,
				price=v.price,time=time,nname=seller,unformattedtime = v.expire,
				copies = v.cnt,unit = unit,qty = qty,
			...
			elseif(arg1 == "宠物口粮")then
				if(v.gsid == 17168 or v.gsid == 17144 or v.gsid == 17018)then
				else
				table_insert(self.DisplayItems,{id=v.id,gsid = v.gsid,name=item_name,
				level = level,nid=v.nid,
				price=v.price,time=time,nname=seller,unformattedtime = v.expire,
	...
	ItemsConsignment.UpdateDataSource()
end

function ItemsConsignment.UpdateDataSource() 
...
function ItemsConsignment.AppendToShop(params)
	...
	paraworld.auction.AppendToShop(params,nil,function(msg)  
		...
		if(msg and msg.issuccess)then
			...
			-- the following are auto refreshed: 
			ItemsConsignment.GetInShopByMe({nid=nil,pindex=0,psize =PAGE_SIZE_SELL});
			-- local clear selled items
			...
			--local last_value = ItemsConsignment.GetItemCopies(ItemsConsignment.SELL_ID);
			--last_value = tonumber(last_value) - commission;

			--ItemsConsignment:SetCtrlValue("txtGoldBean",tostring(last_value))
...
function ItemsConsignment.BuyFromShop(params)
	...
	local cost = ((self.commission or 0)+ price);
	
	local name_text = self.GenDisplayName(item.name, item.copies) or "";
	NPL.load("(gl)script/apps/Aries/Desktop/GUIHelper/CustomMessageBox.lua");

	_guihelper.Custom_MessageBox(string.format( "确认花费%d魔豆购买【%s】？",cost,name_text),function(res) 
		if(self.GetItemCopies(self.MAGICBEAN_ID) < cost)then
			MSG("你的魔豆已不足！")
			return 
		end
		...
		if(res and res == _guihelper.DialogResult.Yes) then
			paraworld.auction.BuyFromShop(params,nil,function(msg)
				if(msg and msg.issuccess)then
					...
					--last_value = tonumber(last_value) - cost - (sell_price or 0);
