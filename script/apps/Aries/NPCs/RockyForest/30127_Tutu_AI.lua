...
script/apps/Aries/NPCs/FollowPets/30127_Tutu_AI.lua
...
-- create class
local libName = "Tutu_AI";
local Tutu_AI = commonlib.gettable("MyCompany.Aries.Quest.NPCAI.Tutu_AI");

-- Tutu_AI framemove
function Tutu_AI.On_FrameMove()
	-- 0.3s interval
	local chick = ParaScene.GetObject(sensor_name);
	local player = ParaScene.GetPlayer();
	
	if(chick:IsValid() == true and player:IsValid() == true) then
		local memory = MyCompany.Aries.Quest.NPCAIMemory.GetMemory(30127);
		...
		if(memory.dist and memory.dist > 5 and dist <= 5) then
			...
			if(targetNPC_id == 30127 and targetNPC_instance ~= instance) then
