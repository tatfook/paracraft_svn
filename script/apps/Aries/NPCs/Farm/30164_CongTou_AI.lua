...
script/apps/Aries/NPCs/FollowPets/30164_CongTou_AI.lua
...
-- create class
local libName = "CongTou_AI";
local CongTou_AI = commonlib.gettable("MyCompany.Aries.Quest.NPCAI.CongTou_AI");

-- CongTou_AI framemove
function CongTou_AI.On_FrameMove()
	-- 0.3s interval
	local mushroom = ParaScene.GetObject(sensor_name);
	local player = ParaScene.GetPlayer();
	
	if(mushroom:IsValid() == true and player:IsValid() == true) then
		local memory = MyCompany.Aries.Quest.NPCAIMemory.GetMemory(30164);
