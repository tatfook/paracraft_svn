...
ChatWindow.DefaultPos = {
	RestoreBtn = {alignment = "_lb", left = 0, top = -254+75, width = 21, height = 24, background = "Texture/Aries/ChatSystem/jiahao_32bits.png;0 0 21 24"},
	LogWnd = {alignment = "_lb", left = 0, top = -400+75-30, width = 320, height = 170+30},
	EditWnd = {alignment = "_lb", left = 0, top = -232+75, width = 320, height = 30},
}
...
function ChatWindow.ToggleChatWindowSize()
...
function ChatWindow.CreateGetParentWnd()
	if(ChatWindow.DefaultPos.ParentWnd) then
		local _parent = ParaUI.GetUIObject("ChatAreaParentWnd");
		if(not _parent:IsValid()) then
			_parent = ParaUI.CreateUIObject("container", "ChatAreaParentWnd", ChatWindow.DefaultPos.ParentWnd.alignment, ChatWindow.DefaultPos.ParentWnd.left, ChatWindow.DefaultPos.ParentWnd.top, ChatWindow.DefaultPos.ParentWnd.width, ChatWindow.DefaultPos.ParentWnd.height);
			_parent.background = "";
			_parent.zorder = -1;
...
function ChatWindow.CreateGetRestoreBtn()
	...
	if(not _parent:IsValid()) then
		...
		_parent = ParaUI.CreateUIObject("button", "RestoreChatWndBtn", ChatWindow.DefaultPos.RestoreBtn.alignment, ChatWindow.DefaultPos.RestoreBtn.left, ChatWindow.DefaultPos.RestoreBtn.top, ChatWindow.DefaultPos.RestoreBtn.width, ChatWindow.DefaultPos.RestoreBtn.height);
		_parent.background = ChatWindow.DefaultPos.RestoreBtn.background;
...
function ChatWindow.OnClickWndRestore()
	...
	if(_parentwnd) then
		...
		_parentwnd.visible = true;
		if(ChatWindow.DefaultPos.ParentWnd) then
			_parentwnd:Reposition(ChatWindow.DefaultPos.ParentWnd.alignment, ChatWindow.DefaultPos.ParentWnd.left, ChatWindow.DefaultPos.ParentWnd.top, ChatWindow.DefaultPos.ParentWnd.width, ChatWindow.DefaultPos.ParentWnd.height)
		end
	end
end
...
function ChatWindow.ShowChatLogPage(bForceRefreshPage, alignment, left, top, width, height)
	if(bForceRefreshPage or not ChatWindow.page) then
		ChatWindow.page = Map3DSystem.mcml.PageCtrl:new({
			url=if_else(System.options.version=="kids", "script/apps/Aries/BBSChat/ChatSystem/ChatWindow.html", "script/apps/Aries/BBSChat/ChatSystem/ChatWindow.teen.html"), 
			click_through=true});
		if(System.options.IsMobilePlatform) then
			ChatWindow.page.SelfPaint = true;
		end
	end
	...
	if(not _parent or not _parent:IsValid()) then
		_parent = ParaUI.CreateUIObject("container", "ChatLogPage", alignment or ChatWindow.DefaultPos.LogWnd.alignment, left or ChatWindow.DefaultPos.LogWnd.left, top or ChatWindow.DefaultPos.LogWnd.top, width or ChatWindow.DefaultPos.LogWnd.width, height or ChatWindow.DefaultPos.LogWnd.height);
		_parent.background = "";
		_parent.visible=false;
		_parent.zorder = -3;
		_parent:GetAttributeObject():SetField("ClickThrough", true);
...
function ChatWindow.FadeIn()
	if(ChatWindow.is_fade_out) then
		ChatWindow.is_fade_out = false;
		if(ChatWindow.page) then
			local _parent = ChatWindow.page:FindControl("canvas");
			--local _parent = ParaUI.GetUIObject("ChatLogPage");
			UIAnimManager.ChangeAlpha("Aries.ChatWindow", _parent, 255, 512)
		end
	end
end

function ChatWindow.FadeOut()
	if(not ChatWindow.is_fade_out) then
		ChatWindow.is_fade_out = true;
		if(ChatWindow.page) then
			local _parent = ChatWindow.page:FindControl("canvas");
			--local _parent = ParaUI.GetUIObject("ChatLogPage");
			UIAnimManager.ChangeAlpha("Aries.ChatWindow", _parent, if_else(System.options.version == "teen", 0, 0), 64, 2000)
		end
	end
end
...
function ChatWindow.CreateDragger(param, mcmlNode)
	...
	_this:SetScript("ondragmove", function(ui_obj)
		...
		if(x<0) then x=0; end
		if(y<0) then y=0; end
		_parentwnd:Reposition("_lt", x, y-(ChatWindow.DefaultPos.dragger_top or 49), ChatWindow.DefaultPos.ParentWnd.width, ChatWindow.DefaultPos.ParentWnd.height)
	end);
...
function ChatWindow.DrawTextNodeHandler(_parent, treeNode)
	...
	if(System.options.mc) then
		fromschool = ""
		toschool = ""
		if(from==System.App.profiles.ProfileManager.GetNID()) then
			fromname = "我";
		end
	else
	...
	if(from==System.App.profiles.ProfileManager.GetNID() and chatdata.to)then
	...
	elseif(from and not chatdata.to)then
		...
		if(fromschool ~= "") then
			...
			if(System.options.mc) then
				mcmlStr = string.format([[<div style="float:left;color:#ff0000">%s</div><span><font color="ffffff">%s</font></span>]],
					fromname..if_else(chatdata.bHideColon, "", "："), words);
			else
				mcmlStr = string.format([[<div style="line-height:14px;font-size:12px;color:#%s;" %s>%s<div style="float:left;"><a 
					tooltip="%s" style="margin-left:0px;float:left;height:12px;background:url()" name="x" 
					onclick="MyCompany.Aries.ChatSystem.ChatWindow.OnClickName" param1='%s'>
					<div style="float:left;margin-top:-2px;color:#%s;">%s</div></a><span>%s</span></div></div>]],
					chatdata.color, use_shadow, if_else(bEmptyChannelName, chatdata.channelname, "["..chatdata.channelname.."]"),  
					if_else(chatdata.bHideTooltip, "", "左键私聊, 右键查看"), 
					tostring(chatdata.from)..":"..fromname,chatdata.color, "["..fromname.."]"..if_else(chatdata.bHideColon, "", "："), words);
			end
	...
	elseif(from) then
		mcmlStr = string.format([[<div style="line-height:14px;font-size:12px;color:#%s;" %s>[%s][unknown]%s</div>]],chatdata.color,use_shadow,chatdata.channelname, words);
	else
...
function ChatWindow.NotifyRepaint()
	if (ChatWindow.page) then	
		ChatWindow.page:InvalidateRect();
	end
end
