...
MyCompany.Aries.ChatSystem.ChatEdit.ShowPage();
-------------------------------------------------------
]]
...
function ChatEdit.ShowPage(bForceRefreshPage, alignment, left, top, width, height)
	if(bForceRefreshPage or not ChatEdit.page) then
		ChatEdit.page = Map3DSystem.mcml.PageCtrl:new({
			url=if_else(System.options.version=="kids", "script/apps/Aries/BBSChat/ChatSystem/ChatEdit.html", "script/apps/Aries/BBSChat/ChatSystem/ChatEdit.teen.html"), 
			click_through=true});
	end
	...
	if(not ChatEdit.redirect_UI_name) then
		...
		if(not _parent or not _parent:IsValid()) then
			_parent = ParaUI.CreateUIObject("container", "ChatEditPage", alignment or ChatWindow.DefaultPos.EditWnd.alignment, left or ChatWindow.DefaultPos.EditWnd.left, top or ChatWindow.DefaultPos.EditWnd.top, width or ChatWindow.DefaultPos.EditWnd.width, height or ChatWindow.DefaultPos.EditWnd.height);
			_parent.background = "";
			...
			ChatEdit.OnClickChannel(ChatEdit.selected_channel, "附近");
...
function ChatEdit.FadeIn()
	if(ChatEdit.is_fade_out) then
		ChatEdit.is_fade_out = false;
		if(not ChatEdit.redirect_UI_name) then
			local _parent = ParaUI.GetUIObject("ChatEditPage");
			UIAnimManager.ChangeAlpha("Aries.ChatEditPage", _parent, 255, 512)
		end
	end
end

function ChatEdit.FadeOut()
	if(not ChatEdit.is_fade_out) then
		ChatEdit.is_fade_out = true;
		if(not ChatEdit.redirect_UI_name) then
			local _parent = ParaUI.GetUIObject("ChatEditPage");
			UIAnimManager.ChangeAlpha("Aries.ChatEditPage", _parent, if_else(System.options.version == "teen", 0, 0), 64, 2000)
		end
	end
end
...
--		local x,y = _editbox:GetAbsPosition();
		...
		local x,y = _editbox:GetAbsPosition();
		CommandHelpPage.HandlerKeyUp({x = x,y = y,rows = 5,offset_x = -40,offset_y = -13},virtual_key,callbacks,sentText);
