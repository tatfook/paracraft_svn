...
Use Lib:
-------------------------------------------------------
NPL.load("(gl)script/apps/Aries/Desktop/CreationTab.lua");
-------------------------------------------------------
]]
local CreationTab = {};
commonlib.setfield("MyCompany.Aries.CreationTab", CreationTab)
...
function CreationTab.OnInit()
	...
	commonlib.setfield("MyCompany.Aries.RibbonControl.OnShowCreationTab", function (bShow)
		if(bShow) then
		...
		if(obj) then 
			...
			if(not CreationTab.rcEditObjTabs) then
				...
				CreationTab.rcEditObjTabs = CommonCtrl.RibbonControl:new({
					name = "Aries.Creation.SelectRibbonControl",
					alignment = "_lb",
					left = 350+3,
					top = -35-46-38-4,
					width = 500,
					...
					parent = nil,
					tabs = {
						["CharEditTab"] = {file="script/apps/Aries/Roles/CharEditTab.html", onshow=nil, },
						["ModelEditTab"] = {file="script/apps/Aries/Roles/ModelEditTab.html", onshow=nil, },
					},
				});
			end
			...
			if(obj:IsCharacter()) then
				...
				-- update name
				local _this = ParaUI.GetUIObject("Aries_edit_char_objname");
				...
				-- update name
				local _this = ParaUI.GetUIObject("Aries_edit_model_objname");
				if(_this:IsValid()) then
					local name, assetname = GetAssetDisplayName(obj)
					_this.text, _this.tooltip = name, "点击查看:"..tostring(assetname);
				end
...
function CreationTab.OnSelectionInfo()
	System.App.Commands.Call("File.MCMLWindowFrame", {
		url="script/apps/Aries/Assets/AssetInfoViewer.html",
