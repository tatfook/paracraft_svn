...
function Calendar.Init()
	local self = Calendar;
	local config_file="config/Aries/Others/calendar.teen.xml"; 
	...
	for week_set in commonlib.XPath.eachNode(xmlRoot, "/Calendar/Week") do
		...
		for each_act in commonlib.XPath.eachNode(week_set, "/act") do
			...
			local act_gametype = string.lower(each_act.attr.gametype or "");
			local act_desc = each_act.attr.desc;
			local act_times = each_act.attr.times or -1;
			act_times = tonumber(act_times);
			local act_place = each_act.attr.place;
			local act_npcid = tonumber(each_act.attr.npcid);
			local act_gsid = each_act.attr.actgsid or -2;
			act_gsid = tonumber(act_gsid);
			...
			local act_key = each_act.attr.keywords;
			...
			if (act_doact=="petfeed") then
				...
				CombatPetPane.SearchPetList(Map3DSystem.User.nid,function(msg)
			...
			if (act_combatlvl_string=="-1") then
				act_combatlvl_string="任意等级";
			end
			...
			local act={name=act_name,worldname=act_worldname,
						...
						doact=act_doact,keywords=act_key,tag=act_tag};
...
--	commonlib.echo("============calendar_map");		
	--commonlib.echo(self.datasource_map);	
	--commonlib.echo(self.HasPet);	
end
...
function Calendar.CreatePage(leavehaqi,paraoldv)
	...
	if (leavehaqi and paraoldv) then
		params = {
				url = "script/apps/Aries/Desktop/Calendar.teen.html?leavehaqi=1&paraoldv="..paraoldv,  
		...
		params = {
				url = "script/apps/Aries/Desktop/Calendar.teen.html", 
				name = "Calendar.ShowPage", 
				app_key=MyCompany.Aries.app.app_key, 
				isShowTitleBar = false,
				DestroyOnClose = true, -- prevent many ViewProfile pages staying in memory
...
-- @result 0: 等级不足, 1: 等级满足、当前也在活动时间内，2:等级满足、还未到活动时间, 3:时间已过, 4:没有战宠，5：今天已完成
function Calendar.GetState(index)
	if (next(Calendar.datasource[index])~=nil) then
		...
		local _mlvl = tonumber(MyCompany.Aries.Player.GetVipLevel() or "0");
		local _actgsid = tonumber(Calendar.datasource[index].actgsid);
		local _doact = Calendar.datasource[index].doact;

		-- 如果是魔法星活动
		if (_actgsid == 50316) then
			if (_mlvl<=0) then
				return 0;
			end
		end
		...
		if (_actgsid>0) then
			local _take = ItemManager.GlobalStoreObtainCounts[_actgsid];        
			local _lefttimes = _take.inday;
			local _totaltimes = tonumber(Calendar.datasource[index].times);
			if (_lefttimes==0 and _totaltimes>0 ) then
				return 5
			end
		end
		...
		-- 如果是战宠喂养
		if (_doact) then
			if (_doact == "petfeed") then
				if (not Calendar.HasPet) then
					return 4;
				end
			end
		end
...
function Calendar.RecommFilter(isrefresh)
	local self = Calendar;
