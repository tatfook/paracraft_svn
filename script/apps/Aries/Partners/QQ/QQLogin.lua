...
function QQLogin.OnInit()
end

function QQLogin.OnClosed()
end
...
--  it defaults to QQLogin.OnProcessResultDefault
function QQLogin.ShowPage(url, callback)
	callback = callback or QQLogin.OnProcessResultDefault;
	QQLogin.url = url;
	QQLogin.callback = callback;
	QQLogin.result = {};

	local width, height = 960, 560;
	local params = {
		url = "script/apps/Aries/Partners/QQ/QQLogin.html", 
		name = "QQ.QQLoginPage", 
	...
	params._page.OnClose = function()
		if(callback) then
			callback(QQLogin.result)
		end
	end
end

-- bind current user to qq account.
function QQLogin.OnProcessResultDefault(result)
	if(not result) then
		return
	end
	LOG.std(nil, "system", "QQLogin.result", result);
	if(result.errorcode) then
		_guihelper.MessageBox("认证失败了, 请重新尝试");
	elseif(result.uid and result.token and tonumber(result.plat) == Platforms.PLATS.QQ) then
		...
		if _nid < 0 then -- 未与NID绑定
			if (System.User.IsAuthenticated) then -- 当前为登录状态
				...
				_guihelper.MessageBox("我们需要将您的哈奇角色与QQ帐号建立关联, 方便您下次分享. 是否同意？", function()
					paraworld.users.NIDRelationOtherAccount({plat = _plat, oid = _uid}, "users.NIDRelationOtherAccount", function(msg)
						...
						if _err == 0 then -- 帐户绑定成功
							_bl = true;
							-- TODO: 给用户提示
							_guihelper.MessageBox("您的角色已经成功绑定QQ号, 可以开始分享了！");
							OnLoggedIn_()
						elseif _err == 417 then -- 该NID已绑定过此平台帐户
							-- TODO: 给用户提示
							_guihelper.MessageBox("您的角色已绑定过其他QQ帐户，请用您绑定的QQ号登陆");
						elseif _err == 433 then -- 该平台帐户已被其它NID绑定
							-- TODO: 给用户提示
							_guihelper.MessageBox("您输入的QQ帐号已经与其他哈奇角色绑定过了, 请用其他QQ号分享");
						else -- 其它错误
							-- TODO: 给用户提示
			...
			if System.User.IsAuthenticated then -- 当前为登录状态
				if _nid == tonumber(Map3DSystem.User.nid) then
					...
					_guihelper.MessageBox("您刚刚输入的QQ账号已经与其它角色绑定过了！目前暂时不支持一个QQ号绑定多个角色～");
				end
			else
