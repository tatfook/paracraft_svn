...
Author(s): LiXizhi
Date: 2009/8/1
Desc:  script/apps/Aries/Login/UserLoginPage.html?username=&password=
...
local UserLoginPage = commonlib.gettable("MyCompany.Aries.UserLoginPage");

--是否处于服务器关闭时间，如果是 隐藏此页面，只显示服务器关闭提醒
UserLoginPage.isClosedTime = false; 
...
-- init
function UserLoginPage.OnInit()
	if(System.options.version == "kids" and System.options.login_tokens and System.options.login_tokens.platid=="2144") then
		if(not UserLoginPage.stats_login_sent) then
			UserLoginPage.stats_login_sent = true;
			local tokens = System.options.login_tokens;
			local params = {
				domain="web.2144.cn",
				url="http://web.2144.cn/client/haqi",
				title="2144-魔法哈奇微端",
				account = "serverVisit",
				uid = tokens.id or tokens.userid or tokens.nid,
				sid = 24,
				gid = 3, 
				--time = tokens.time,
				time = os.time()*1000,
			};
			local url = System.localserver.UrlHelper.BuildURLQuery("http://trace2144.2144.cn/__beacon.gif", params);
			NPL.AppendURLRequest(url, "echo(msg);", nil, "r");
		end
	end
	...
	if(ParaIO.DoesFileExist("temp/instant_login/username", true) == true) then
		if(ParaIO.DoesFileExist("temp/instant_login/password", true) == true) then
			...
			UserLoginPage.OnClickLogin(btnName, {
				user_name = user_name,
				password = password,
...
function UserLoginPage.OnClickBackToLocalUserSelect()
	if(MainLogin.state.local_user) then
...
function UserLoginPage.OnClickRegister(realnm)
	if(page) then
		page:CloseWindow();
	end
	if (realnm==1) then
...
function UserLoginPage.OnClickChangePassword()
...
function UserLoginPage.OnClickForgetPassword()
	NPL.load("(gl)script/apps/Aries/Login/ExternalUserModule.lua");
	local ExternalUserModule = commonlib.gettable("MyCompany.Aries.ExternalUserModule");
	local cfg = ExternalUserModule:GetConfig();
...
function UserLoginPage.OnClickProtectPassword()
	NPL.load("(gl)script/apps/Aries/Login/ExternalUserModule.lua");
	local ExternalUserModule = commonlib.gettable("MyCompany.Aries.ExternalUserModule");
	local cfg = ExternalUserModule:GetConfig();
...
function UserLoginPage.OnCheckRememberUsername(bChecked)
...
function UserLoginPage.OnCheckRememberPassword(bChecked)
...
function UserLoginPage.OnClickLogin(btnName, forms)
	...
	if(System.options.login_tokens) then
		...
		MainLogin:next_step({IsLoginStarted=true, auth_user = {
			username=tokens.id or tokens.userid or tokens.nid, password=tokens.password,
	...
	local user_name = string.gsub(forms.user_name, "^%s*(.-)%s*$", "%1");
	...
	if(user_nid) then
		...
		if (region_config.is_username_email) then
			...
			if(domainPart == "paraengine.com") then
				-- strip domain part 
				user_name = userPart
				commonlib.applog("UserLoginPage's domain part is ignored. Remove this in release build. ")
			end
		end		
	end
