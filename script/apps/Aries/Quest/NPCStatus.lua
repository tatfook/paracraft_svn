...
use the lib:
------------------------------------------------------------
NPL.load("(gl)script/apps/Aries/Quest/Quest_NPCStatus.lua");
------------------------------------------------------------
]]
...
-- create class
local Quest_NPCStatus = commonlib.gettable("MyCompany.Aries.Quest_NPCStatus");
...
local texture_assets = {
	...
	["DIALOG_STATUS_UNAVAILABLE"] = "",
	["DIALOG_STATUS_CHAT"] = "",
	["DIALOG_STATUS_INCOMPLETE"] = "Texture/Aries/Minimap/Question_Mark_Grey_32bits.png",
	["DIALOG_STATUS_REWARD_REP"] = "",
	["DIALOG_STATUS_AVAILABLE_REP"] = "",
	["DIALOG_STATUS_AVAILABLE"] = "Texture/Aries/Minimap/Excalmatory_Mark_32bits.png",
	["DIALOG_STATUS_REWARD2"] = "", 
	["DIALOG_STATUS_REWARD"] = "Texture/Aries/Minimap/Question_Mark_32bits.png", 
}
...
function Quest_NPCStatus.SetCMBCreditCard(bSet)
	...
	if(Quest_NPCStatus.hasCMBCreditCard == false and bSet == true) then
		MyCompany.Aries.Desktop.Dock.ShowNotification("您开通了招商银行信用卡\n");
	end
...
function Quest_NPCStatus.SetCMBBankAccount(bSet)
	...
	if(Quest_NPCStatus.hasCMBBankAccount == false and bSet == true) then
		MyCompany.Aries.Desktop.Dock.ShowNotification("您开通了一卡通帐户\n");
	end
...
function Quest_NPCStatus.OnReceiveNearbyNPC(NPC)
	...
	player:SetField("Sentient Radius", System.options.CharClickDistSq); -- sense click distance characters
	...
	if(NPC.Name == "帕拉巫世界传送大使") then
		local player = ParaScene.GetCharacter("NPC:"..NPC.Name);
		System.UI.MiniMapManager.RegisterOPCObject(player.name, "model/test/ryb/blue/blue.x", "Texture/Aries/Minimap/Portal_32bits.png");
		...
		FSM:SetFinalCallback(function(state)
			...
			if(state == 2) then
				...
				System.App.Commands.Call("File.EnterAriesWorld", {worldpath = "worlds/MyWorlds/AlphaWorld", role = "guest"});
			elseif(state == 3) then
				log("DO: Doodle World transport\n")
				System.App.Commands.Call("File.EnterAriesWorld", {worldpath = "worlds/MyWorlds/DoodleWorld"});
			end
		end);
	...
	elseif(NPC.Name == "IKEA营业员Anna") then
		...
		FSM:SetStateEntryAction(2, function() 
			MyCompany.Aries.Quest_DialogWnd.ShowFSMDialog(NPC.ID, 2);
		end);
		
		FSM:SetStateEntryAction(3, function() 
			MyCompany.Aries.Quest_DialogWnd.ShowFSMDialog(NPC.ID, 3);
		end);
		...
		FSM:SetFinalCallback(function(state)
			if(state == 4) then
				...
				System.App.Commands.Call("File.MCMLWindowFrame", {
					url = "script/apps/Aries/Quest/IKEA/BrowseProduct.html", 
				...
				System.App.Commands.Call("Profile.Aries.ShowAssetBag", {url = "script/apps/Aries/Bag/MyBag.html"});
			elseif(state == 5) then
				...
				MyCompany.Aries.Desktop.Dock.ShowNotification(function (_parent)
					...
					_notify.background = "Texture/Aries/Andy/CreditCard.png";
				...
				System.App.Commands.Call("File.MCMLWindowFrame", {
					url = "script/apps/Aries/Quest/IKEA/BrowseDiscountProduct.html", 
					name = "CMB.OpenAccount", 
					app_key = MyCompany.Aries.app.app_key, 
					text = "招行信用卡专用通道",
					DestroyOnClose = true,
				...
				System.App.Commands.Call("Profile.Aries.ShowAssetBag", {url = "script/apps/Aries/Bag/MyBagWithCreditCard.html"});
			elseif(state == 6) then
				System.Quest.Client.QuestgiverBye();
				log("DO: Leave\n")
			end
	...
	elseif(NPC.Name == "招商银行营业员小丽" or NPC.Name == "招商银行营业员小月") then
		...
		FSM:SetStateEntryAction(4, function() 
			MyCompany.Aries.Quest_DialogWnd.ShowFSMDialog(NPC.ID, 4);
		end);
		
		FSM:SetStateEntryAction(5, function() 
			MyCompany.Aries.Quest_DialogWnd.ShowFSMDialog(NPC.ID, 5);
		end);
		
		FSM:SetStateEntryAction(6, function() 
			MyCompany.Aries.Quest_DialogWnd.ShowFSMDialog(NPC.ID, 6);
		end);
		...
		FSM:SetFinalCallback(function(state)
			if(state == 7) then
				...
				log("DO: 进入存取款界面\n")
				System.App.Commands.Call("File.MCMLWindowFrame", {
					url = "script/apps/Aries/Quest/CMB/ATM.html", 
					name = "CMB.ATM", 
					app_key = MyCompany.Aries.app.app_key, 
					text = "招商银行ATM机",
					DestroyOnClose = true,
			...
			elseif(state == 8) then
				...
				log("DO: 进入银行开户流程\n")
				System.App.Commands.Call("File.MCMLWindowFrame", {
					url = "script/apps/Aries/Quest/CMB/OpenAccount.html", 
					name = "CMB.OpenAccount", 
					app_key = MyCompany.Aries.app.app_key, 
					text = "招商银行一卡通",
					DestroyOnClose = true,
			...
			elseif(state == 9) then
				...
				log("DO: 打开活动内容介绍页面1\n")
				System.App.Commands.Call("File.MCMLWindowFrame", {
					url = "script/apps/Aries/Quest/CMB/Special1.html", 
					name = "CMB.Special1", 
					app_key = MyCompany.Aries.app.app_key, 
			...
			elseif(state == 10) then
				...
				log("DO: 打开活动内容介绍页面2\n")
				System.App.Commands.Call("File.MCMLWindowFrame", {
					url = "script/apps/Aries/Quest/CMB/Special2.html", 
					name = "CMB.Special2", 
					app_key = MyCompany.Aries.app.app_key, 
			...
			elseif(state == 11) then
				...
				log("DO: 打开活动内容介绍页面3\n")
				System.App.Commands.Call("File.MCMLWindowFrame", {
					url = "script/apps/Aries/Quest/CMB/Special3.html", 
					name = "CMB.Special3", 
					app_key = MyCompany.Aries.app.app_key, 
			...
			elseif(state == 12) then
				...
				log("DO: 进入信用卡申请流程\n")
				System.App.Commands.Call("File.MCMLWindowFrame", {
					url = "script/apps/Aries/Quest/CMB/ApplyCreditCard.html", 
	...
	elseif(NPC.Name == "电信营业员小李" or NPC.Name == "电信营业员小张") then
		...
		FSM:SetFinalCallback(function(state)
			if(state == 4) then
				...
				log("DO: 打开189抢号界面\n")
				System.App.Commands.Call("File.MCMLWindowFrame", {
					url = "script/apps/Aries/Quest/Telecom/189.html", 
					name = "Telecom.189", 
					app_key = MyCompany.Aries.app.app_key, 
					text = "“天翼”189抢号大行动开始啦！",
					DestroyOnClose = true,
			...
			elseif(state == 5) then
				...
				log("DO: 打开活动内容介绍页面1\n")
				System.App.Commands.Call("File.MCMLWindowFrame", {
					url = "script/apps/Aries/Quest/Telecom/Special1.html", 
			...
			elseif(state == 6) then
				...
				log("DO: 打开活动内容介绍页面2\n")
				System.App.Commands.Call("File.MCMLWindowFrame", {
					url = "script/apps/Aries/Quest/Telecom/Special2.html", 
					name = "Telecom.Special1", 
					app_key = MyCompany.Aries.app.app_key, 
					text = "“我的e家”宽带1年＝16个月",
					DestroyOnClose = true,
			...
			elseif(state == 7) then
				...
				log("DO: 打开活动内容介绍页面3\n")
				System.App.Commands.Call("File.MCMLWindowFrame", {
					url = "script/apps/Aries/Quest/Telecom/Special3.html", 
					name = "Telecom.Special1", 
					app_key = MyCompany.Aries.app.app_key, 
	...
	elseif(NPC.Name == "户籍管理员清颖") then
		...
		FSM:SetStateEntryAction(4, function() 
			...
			-- notification
			MyCompany.Aries.Desktop.Dock.ShowNotification("您成为了Pala5的正式移民");
			MyCompany.Aries.Quest_DialogWnd.ShowFSMDialog(NPC.ID, 4);
		end);
	...
	elseif(NPC.Name == "一号新手接待员甜甜" or NPC.Name == "二号新手接待员微笑" or NPC.Name == "四号新手接待员百合") then
		...
		FSM:SetStateEntryAction(6, function() 
			System.Quest.Client.QuestgiverBye();
			MyCompany.Aries.Desktop.HideAllVisible();
			System.App.Commands.Call("File.PlayMovieScript", "movie_1.xml");
			
			System.Movie.MoviePlayerPage.PlayerClose = function() MyCompany.Aries.Desktop.RestoreAllVisible(); end;
		end);
