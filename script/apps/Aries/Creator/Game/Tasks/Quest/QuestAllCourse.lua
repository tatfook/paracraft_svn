...
function QuestAllCourse.ShowView()
    ...
    QuestAllCourse.RefreshCourseListData(open_callback)
...
function QuestAllCourse.RefreshLevelListData()
    ...
    local all_course_data = commonlib.copy(select_teacher_data.children[1])
            ...
            for k, v in pairs(QuestAllCourse.CourseListData) do
                ...
                if v.preCourseIds and #v.preCourseIds > 0 then
                    ...
                    if v.IsLock then
                        ...
                        v.lock_desc = string.format("需要完成%s学习才能解锁此课程哦", name_desc)
                    end
                end
    ...
    if data and data.projectId then
        ...
        local function endter_world()
            keepwork.quest_complete_course.get({
                ...
                if err2 == 200 then
                    ...
                    -- commonlib.TimerManager.SetTimeout(function()
                    --     GameLogic.GetFilters():apply_filters("AICourse.ActivateHomework", data.projectId)
                    --     commonlib.TimerManager.SetTimeout(function()
                    --         local step = GameLogic.GetFilters():apply_filters("AICourse.GetStep")
                    --         commonlib.TimerManager.SetTimeout(function()
                    --             step = step + 1
                    --             if step == 3 then
                    --                 step = 10
                    --             end
                    --             GameLogic.GetFilters():apply_filters("AICourse.SetStep", step, 3)
        ...
        if System.User.isVip then
            ...
            if client_data.play_course_times <= 0 then
                ...
                QuestMessageBox.Show(desc, endter_world)
            else
                local course_world_id = client_data.course_world_id or 0
                course_world_id = tonumber(course_world_id)
                -- 使用过之后 如果id相同 则可进入
    ...
    keepwork.quest_complete_homework.get({
        ...
        if err == 200 then
            ...
            if userAiHomework == nil then
                GameLogic.AddBBS(nil, L"请先完成对应课程", 5000, "255 0 0");
                return
            end
            ...
            elseif type == 1 then
                ...
                if work_data.projectId then
                    ...
                    if data.userAiHomework and data.userAiHomework.progress then
                        client_data.course_step = data.userAiHomework.progress.stepNum or 0
                    end
