...
<div style="width:%dpx;height:100px;margin-left:%dpx;margin-top:-100px;">
	<div align="center" valign="bottom" style="background:url(%s);min-width:40px;color:%s;font-size:%dpx;padding:%dpx;padding-bottom:%dpx;">%%s</div>
</div>
</pe:mcml>]], maxWidth, -maxWidth/2, bg, text_color, fontSize, headon_speech.padding, headon_speech.padding_bottom);
			end
			self:SetHeadOnDisplay({url=ParaXML.LuaXML_ParseString(format(sHeadonSayTemplate, text))}, 1)
			if(duration > 0) then
