...
commonlib.setfield("MyCompany.Orion.ChatWnd", ChatWnd);
...
function ChatWnd.Show(bShow)
	...
	if(_this:IsValid())then
		...
		_this.background = "Texture/Orion/mainbar.png;0 0 29 29: 8 25 8 3";
		_this.zorder = 5; -- make it stay on top. 
		_this.onframemove = ";MyCompany.Orion.ChatWnd.DoFramemove();";
		_this:AttachToRoot();
		_parent = _this;
		
		-- close button
		_this = ParaUI.CreateUIObject("button", "close", "_rt", -20, 4, 16, 16);
		_this.background = "Texture/Orion/mainbar.png;77 12 16 16";
		_this.onclick = ";MyCompany.Orion.ChatWnd.Show();";
		_parent:AddChild(_this);
		
		-- separator
		_this = ParaUI.CreateUIObject("button", "separator", "_lt", 60, 0, 4, 21);
		_this.background = "Texture/Orion/mainbar.png;34 7 4 21";
		_parent:AddChild(_this);
		...
		_this.onclick = ";MyCompany.Orion.ChatWnd.ChangeFocusChannel(\"Channel_Public\");"
		_parent:AddChild(_this);
		_this = ParaUI.CreateUIObject("button", "HelpChannel", "_lt", 64, 0, 60, 21);
		_this.background = "";
		_this.text = "帮助频道";
		_this.onclick = ";MyCompany.Orion.ChatWnd.ChangeFocusChannel(\"Channel_Help\");"
		_parent:AddChild(_this);
		_this = ParaUI.CreateUIObject("button", "WorldChannel", "_lt", 128, 0, 60, 21);
		_this.background = "";
		_this.text = "世界频道";
		_this.onclick = ";MyCompany.Orion.ChatWnd.ChangeFocusChannel(\"Channel_World\");"
		_parent:AddChild(_this);
		...
		local ctl = CommonCtrl.GetControl("Orion_ChannelPage_TreeView");
		if(ctl == nil) then
			ctl = CommonCtrl.TreeView:new{
				name = "Orion_ChannelPage_TreeView",
				alignment = "_fi",
				left = 2,
				top = 24,
				width = 2,
...
--
	...
	--for k, v in ipairs(ChatWnd.channels) do
		...
		--end
		--
		--paraworld.lobby.GetBBS(msg, "Orion_GetLobbyBBSChannelMessages"..channelName, function(msg)
