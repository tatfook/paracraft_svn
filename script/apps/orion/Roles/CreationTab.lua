...
Use Lib:
-------------------------------------------------------
NPL.load("(gl)script/apps/Orion/Desktop/CreationTab.lua");
-------------------------------------------------------
]]
local CreationTab = {};
commonlib.setfield("MyCompany.Orion.CreationTab", CreationTab)
...
function CreationTab.OnInit()
	...
	commonlib.setfield("MyCompany.Orion.RibbonControl.OnShowCreationTab", function (bShow)
		if(bShow) then
		...
		if(obj) then 
			...
			if(not CreationTab.rcEditObjTabs) then
				...
				CreationTab.rcEditObjTabs = CommonCtrl.RibbonControl:new({
					name = "Orion.Creation.SelectRibbonControl",
					alignment = "_lb",
					left = 350+3,
					top = -35-46-38-4,
					width = 500,
					...
					parent = nil,
					tabs = {
						["CharEditTab"] = {file="script/apps/Orion/Roles/CharEditTab.html", onshow=nil, },
						["ModelEditTab"] = {file="script/apps/Orion/Roles/ModelEditTab.html", onshow=nil, },
					},
				});
			end
			...
			if(obj:IsCharacter()) then
				...
				-- update name
				local _this = ParaUI.GetUIObject("Orion_edit_char_objname");
				...
				-- update name
				local _this = ParaUI.GetUIObject("Orion_edit_model_objname");
				if(_this:IsValid()) then
					local name, assetname = GetAssetDisplayName(obj)
					_this.text, _this.tooltip = name, "点击查看:"..tostring(assetname);
				end
...
function CreationTab.OnSelectionInfo()
	System.App.Commands.Call("File.MCMLWindowFrame", {
		url="script/apps/Orion/Assets/AssetInfoViewer.html",
		name = "HelloModelViewer",
		app_key = MyCompany.Orion.app.app_key, 
		text = "模型信息",
