...
function EBook_MediaMenu.OnClickDrawing()
	...
	if(snapshot~=nil) then
		...
		KidsPainter.imagesize = 256; -- should be larger
		KidsPainter.OnCloseCallBack = nil;
		KidsPainter.OnSaveCallBack = EBook_MediaMenu.OnClickSaveDrawing;
		KidsPainter.ShowPainter(true, "_lt", 0,0);
		KidsPainter.LoadFromTexture(snapshot);
	end
end
...
function EBook_MediaMenu.OnClickSaveDrawing()
	local snapshot = EBook_db.GetPageScreenShotPath();
	if(snapshot~=nil) then
		if(KidsPainter.SaveAs(snapshot)) then
