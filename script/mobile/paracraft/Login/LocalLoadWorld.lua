...
LocalLoadWorld.selected_world_index = 1;
...
function LocalLoadWorld.ShowPage()
	...
	System.App.Commands.Call("File.MCMLWindowFrame", {
			url = "script/mobile/paracraft/Login/LocalLoadWorld.html?folder=worlds/DesignHouse", 
...
function LocalLoadWorld.OnClickOpenWorldDirectory()
	local full_path = ParaIO.GetWritablePath()..LocalLoadWorld.GetWorldFolder();
	-- TODO: 
	_guihelper.MessageBox(full_path);
end

-- refresh all causing world list to be refreshed. 
function LocalLoadWorld.RefreshAll()
	if(page) then
		LocalLoadWorld.IsWorldListLoaded = nil;
		LocalLoadWorld.page:Rebuild();
	end
end
					...
					for node in commonlib.XPath.eachNode(xmlRoot, "/pe:mcml/pe:world") do
						if(node.attr) then
							...
							LocalLoadWorld.AddWorldToDS({worldpath = folderPath.."/"..item.filename, 
								...
								createdate = node.attr.create_date,
								order = item.order,
...
function LocalLoadWorld.OnClickDeleteWorld()
	local world = LocalLoadWorld.dsWorlds[LocalLoadWorld.selected_world_index];
	...
	if(world) then
		...
		if(string.find(filepath, "^worlds/")) then
			...
			if(dirPath)then
				local text = string.format(L"确定删除世界:%s?", world.Title or "")
				LOG.std(nil, "info", "LocalLoadWorld", "ask to delete world %s", world.Title or "");
				_guihelper.MessageBox(text, 
					function ()
						local targetDir = dirPath;
						if(commonlib.Files.DeleteFolder(targetDir)) then 
							LOG.std(nil, "info", "LocalLoadWorld", "world dir deleted: %s ", targetDir);
							LocalLoadWorld.RefreshAll();
						else
			...
			_guihelper.MessageBox(L"您没有权限删除这个世界, 你可以用外部文件浏览器手工删除");
		end	
	else
		_guihelper.MessageBox(L"请选择一个世界");
	end	
end

function LocalLoadWorld.OnSwitchWorld(name)
	LocalLoadWorld.selected_world_index = tonumber(name);
	LocalLoadWorld.page:Refresh(0.01);
end

function LocalLoadWorld.OnClickLoadWorld(name)
	local index = tonumber(name);
	local world = LocalLoadWorld.dsWorlds[index];
	...
	elseif(worldname == "_emptyworld") then
		...
		if(res == true) then
			...
			if(file:IsValid()) then
				...
				-- load success UI
				return worldpath, string.format(L"世界[%s]创建成功! 开始你的创造吧", values.title or worldname);
			else
				_guihelper.MessageBox(L"创建tag.xml出错了")	
			end
