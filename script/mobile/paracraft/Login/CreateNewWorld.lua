...
CreateNewWorld.default_worldname= "DefaultWorld";
...
CreateNewWorld.terrain_info = {
	{index = 1,terrain = "superflat",  show_value = L"超级平坦", description = L"海拔0米,无地下"},
	--{index = 2,terrain = "custom", show_value = L"随机地形", description = "森林，沙漠，雪地，山洞"},
	--{index = 3,terrain = "empty", show_value = L"空", description = "SDK用户可用命令行创建地基"},
	--{index = 4,terrain = "flat",  show_value = L"高原", description = "海拔128米"},
}
...
function CreateNewWorld.ShowPage()
	System.App.Commands.Call("File.MCMLWindowFrame", {
		url = "script/mobile/paracraft/Login/CreateNewWorld.html", 
...
function CreateNewWorld.SearchFiles(output, rootfolder,nMaxFilesNum, filter)
	...
	for i = 0, nCount-1 do 
		...
		output[nextIndex].time_text = string.format(L"%d年%d月%d日(%d点%d分)", year, month, day, hour, mins);
