...
local newuser_tip_text = "点击我， 开始你的创造之旅！";
...
function GoalTracker.OnInit()
	...
	if(GoalTracker.tip_text == "") then
		...
		NPL.load("(gl)script/apps/Aries/Creator/Game/API/ExpTable.lua");
		local ExpTable = commonlib.gettable("MyCompany.Aries.Creator.Game.API.ExpTable");
		if(ExpTable.GetThisLevelExp(profile:GetExp())<=2) then
	...
	profile:GetEvents():AddEventListener("OnBuildProgressChanged", GoalTracker.OnBuildProgressChanged, GoalTracker, "GoalTracker");
...
function GoalTracker.GetExpTip(Exp)
	...
	return format("还需%d xp 到达%d级", levelup_exp, level);
end
...
function GoalTracker:OnBuildProgressChanged(event)
	if(page and event.value) then
		...
		if(btn) then
			...
			ObtainItemEffect:new({background="", text="教学完成", duration=3000, color="#33ff33", width=120,height=25, 
...
function GoalTracker.OnClickCody(name)
	GameLogic.SetTipText(nil);
    NPL.load("(gl)script/apps/Aries/Creator/Game/Tasks/BuildQuestTask.lua");
	MyCompany.Aries.Game.Tasks.BuildQuest.ShowPage();
