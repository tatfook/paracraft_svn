--[[ template<waitseconds = nil> ShopKeeper character
author: WangTian
date: 2006.9.9
desc: ShopKeeper
usage:
==On_Load==[optional]
;NPL.load("(gl)script/AI/templates/ShopKeeper.lua");_AI_templates.ShopKeeper.On_Load();

==On_Perception==
;NPL.load("(gl)script/AI/templates/ShopKeeper.lua");_AI_templates.ShopKeeper.On_Perception();

==On_Click==[optional]
;NPL.load("(gl)script/AI/templates/ShopKeeper.lua");_AI_templates.ShopKeeper.On_Click();

]]
local L = CommonCtrl.Locale("IDE");
if(not _AI_templates) then _AI_templates={}; end
if(not _AI_templates.ShopKeeper) then _AI_templates.ShopKeeper={}; end

--[[ optional
speak some "bla bla bla ..." every 20 seconds using sequence controller
@param waitseconds: if nil, it will be 20.
]]
function _AI_templates.ShopKeeper.On_Load(waitseconds)
	...
	if(player:IsValid() == true) then 
		...
		s:Lable("start");
		s:Wait(waitseconds);
		s:Exec(string.format([[;headon_speech.Speek("%s", "%s", 3);]], sensor_name, L"Today is the first day. Everything is free.."));
		s:Goto("start");
		s:EndAddKeys();
	end
end
...
function _AI_templates.ShopKeeper.On_Perception(waitseconds)
	...
	if(player:IsValid() == true) then 
		...
		if((nTime-mem.LastPerception)<1000) then
			...
			if(mem.LastWait > 1000*waitseconds) then
				headon_speech.Speek(sensor_name, L"Welcome my guest! Come and see my shop.", 4);
...
-- just say something
function _AI_templates.ShopKeeper.On_Click()
	headon_speech.Speek(sensor_name, L"Wanna some drink?", 4);
end